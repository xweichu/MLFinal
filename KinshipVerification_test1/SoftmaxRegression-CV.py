...
def train_NN():
   ...
   # train_step = tf.train.AdamOptimizer(1e-4).minimize(loss)
    ...
    iterations_set =np.arange(5000,90000,5000)
    ...
    for i in range(best_iterations):
        ...
        if i % 20 == 0:
            ...
            writer.add_summary(summary, i)
    correct_prediction = tf.equal(tf.argmax(y_nn, 1), tf.argmax(y_, 1))
    accuracy_tst = tf.reduce_mean(tf.cast(correct_prediction, "float"))
    accuracy_tst= accuracy_tst.eval({x: tr_vectors, y_: tr_labels,keep_prob: 1.0})
    #accuracy_tst= accuracy_tst.eval({x: tst_vectors, y_: tst_labels,keep_prob: 1.0})

    print("whole train data accuracy:%f"%accuracy_tst)
    writer.close()
...
#test data
# tst_vectors,tst_labels =loadInFeatureVect_test()
